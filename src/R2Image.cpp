...
Brighten(double factor)
{
  ...
  for(int i = 0; i<Height(); i++)
  {
  	...
  	{
  		R2Pixel& pixel = Pixel(j, i);
      // Multiplying with the factor to achieve brightness
  		SetPixel(j, i, R2Pixel(pixel.Red()*factor, pixel.Green()*factor, pixel.Blue()*factor, 1.0));
  	}
  }

  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "Brighten(%g) is done\n", factor);
}
...
ChangeContrast(double factor)
{
  ...
  // Calculating mean gray value
  float meanVal = 0.0;
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      R2Pixel& pixel = Pixel(i, j);
      meanVal += (0.3*pixel[0] + 0.59*pixel[1] + 0.11*pixel[2]);
      pixel.Clamp();
    }
  }
  meanVal /= (height*width);

  // Using Complementary filter in RGB and mean gray value;
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      R2Pixel& pixel = Pixel(i, j);

      float newR = pixel[0]*factor + (1-factor)*meanVal;
      float newG = pixel[1]*factor + (1-factor)*meanVal;
      float newB = pixel[2]*factor + (1-factor)*meanVal;
      pixel[0] = newR;
      pixel[1] = newG;
      pixel[2] = newB;
      pixel.Clamp();
	...
	fprintf(stderr, "ChangeContrast(%g) is done\n", factor);
}
...
Blur(double sigma)
{
  ...
  for (int x = -(kSize-1)/2; x <= (kSize-1)/2; x++)
  {
      ...
      {
          r = sqrt(x*x + y*y);
          kernel[x + (kSize-1)/2][y + (kSize-1)/2] = (exp(-(r*r)/s))/(M_PI * s);
          sum += kernel[x + (kSize-1)/2][y + (kSize-1)/2];
      }
  }
  ...
  // Convolution with kernal to blur the image
  for (int j = 0; j < height; j++) 
  {
    for (int i = 0; i < width; i++) 
    {
      ...
      for (int x = -(kSize-1)/2; x <= (kSize-1)/2; x++)
      {
          for(int y = -(kSize-1)/2; y <= (kSize-1)/2; y++)
          {
            ...
            {
                R2Pixel& pixeltemp = Pixel(point[0], point[1]);
                
                sumR += (kernel[x+(kSize-1)/2][y+(kSize-1)/2] * pixeltemp[0]);
                sumG += (kernel[x+(kSize-1)/2][y+(kSize-1)/2] * pixeltemp[1]);
                sumB += (kernel[x+(kSize-1)/2][y+(kSize-1)/2] * pixeltemp[2]);
            }
          }
      }
...
Sharpen()
{
  // Sharpen an image using a gaussian kernel of 5x5
  double kernel[5][5];
  double sigma = 2.5;
  double r, s = 2.0 * sigma * sigma;
  double sum = 0.0;
  ...
  // Convolving with kernel to blur the image
  for (int j = 0; j < height; j++) 
  {
    for (int i = 0; i < width; i++) 
    {
      ...
      for (int x = -2; x <= 2; x++)
      {
          for(int y = -2; y <= 2; y++)
          {
            ...
            if(point[0] >= 0 && point[1] >= 0 && point[0] < width && point[1] < height)
            {
                ...
                sumR += (kernel[x+2][y+2] * pixeltemp[0]);
                sumG += (kernel[x+2][y+2] * pixeltemp[1]);
                sumB += (kernel[x+2][y+2] * pixeltemp[2]);
            }
          }
      }

      // Combining original and blurred image to get sharpened image 
      pixel[0] = 1.5*pixel[0] - 0.5*sumR;
      pixel[1] = 1.5*pixel[1] - 0.5*sumG;
      pixel[2] = 1.5*pixel[2] - 0.5*sumB;
      
      pixel.Clamp();
    }
  }
  fprintf(stderr, "Sharpen() is done\n");
}
...
EdgeDetect(void)
{
  ...
  // Using 3x3 kernel for edge detection {-1,-1,-1,-1,8,-1,-1,-1,-1}
  double kernel[3][3];
  ...
  // Convolving with Edge Filter to find edge detection
  for (int j = 0; j < height; j++) 
  {
    for (int i = 0; i < width; i++) 
    {
      ...
      for (int x = -1; x <= 1; x++)
      {
          for(int y = -1; y <= 1; y++)
          {
            ...
            {
                R2Pixel& pixeltemp = Pixel(point[0], point[1]);
                sumR += (kernel[x+1][y+1] * pixeltemp[0]);
                sumG += (kernel[x+1][y+1] * pixeltemp[1]);
                sumB += (kernel[x+1][y+1] * pixeltemp[2]);
            }
          }
      }

      pixel[0] = std::abs(sumR);
      pixel[1] = std::abs(sumG);
      pixel[2] = std::abs(sumB);
      
      pixel.Clamp();
    }
  }

  fprintf(stderr, "EdgeDetect() is done\n");
}
...
Scale(double sx, double sy, int sampling_method)
{
  ...
  // New image to store image
  R2Image* newimage = new R2Image(width*sx, height*sy);
  ...
  if(sampling_method == 0) // Point sampling
  {
    for (int j = 0; j < height*sy; j++) 
    {
      ...
      {
        R2Pixel& pixel = Pixel(i/sx, j/sy);
        newimage->SetPixel(i, j, R2Pixel(pixel[0], pixel[1], pixel[2], 1));
      }
    }
  ...
  else if (sampling_method == 1) // Bilinear sampling
  {
     ...
     for (int i = 0; i < width*sx; i++)
     {
        for (int j = 0; j < height*sy; j++)
        {
          ...
          double xf = dxpos - ixpos;
          double yf = dypos - iypos;  
          R2Pixel& pixel = Pixel(ixpos, iypos);       

          r1 = xf*Pixels(ixpos)[iypos].Red() 
              + (1 - xf)*Pixels(ixpos+1)[iypos].Red();
          r2 = xf*Pixels(ixpos)[iypos+1].Red() 
              + (1 - xf)*Pixels(ixpos+1)[iypos+1].Red();
          r = yf*r1 + (1 - yf)*r2;


          b1 = xf*Pixels(ixpos)[iypos].Blue() 
               + (1 - xf)*Pixels(ixpos+1)[iypos].Blue();
          b2 = xf*Pixels(ixpos)[iypos+1].Blue() 
               + (1 - xf)*Pixels(ixpos+1)[iypos+1].Blue();
          b = yf*b1 + (1 - yf)*b2;


          g1 = xf*Pixels(ixpos)[iypos].Green() 
                + (1 - xf)*Pixels(ixpos+1)[iypos].Green();
          g2 = xf*Pixels(ixpos)[iypos+1].Green() 
               + (1 - xf)*Pixels(ixpos+1)[iypos+1].Green();
          g = yf*g1 + (1 - yf)*g2;

          newimage->SetPixel(i, j, R2Pixel(r, g, b, 1));
        }
      }
  }


  // else if (sampling_method == 2) // Gaussian Sampling with 3x3 kernel
  // {
  ...
  //   int radius = (size - 1)/2;
  //   double sumR, sumG, sumB, dis;
  //   int xlo, xhi, ylo, yhi, filterposx, filterposy;
  ...
  //     {
  //        dis = abs(radius-i);
  //        kernel[i][j] += exp((-1.0/2)*pow(dis,2.0)/(2*pow((radius/3.),2)));
  //        dis = abs(radius-j);
  //        kernel[i][j] += exp((-1.0/2)*pow(dis,2.0)/(2*pow((radius/3.),2)));
  //        sum += kernel[i][j];
  //     }
  ...
  //                sumR += kernel[filterposx][filterposy]*Pixels(ix)[iy].Red();
  //                sumB += kernel[filterposx][filterposy]*Pixels(ix)[iy].Blue();
  //                sumG += kernel[filterposx][filterposy]*Pixels(ix)[iy].Green();
  //                sum += kernel[filterposx][filterposy];
  //             }
  //          }
  //          sumR /= sum;
  //          sumG /= sum;
  ...
  //          newimage->SetPixel(i, j, R2Pixel(sumR, sumG, sumB, 1));
  //       }
  //     }
  // }
...
Composite(const R2Image& top, int operation)
{
  ...
  // Alpha channel based image blending of two images
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      R2Pixel& to_pixel = Pixel(i, j);
      const R2Pixel& from_pixel = top.Pixel(i, j);
      to_pixel[0] = from_pixel[0]*from_pixel[3] + (1-from_pixel[3])*to_pixel[0];
      to_pixel[1] = from_pixel[1]*from_pixel[3] + (1-from_pixel[3])*to_pixel[1];
      to_pixel[2] = from_pixel[2]*from_pixel[3] + (1-from_pixel[3])*to_pixel[2];
    }
  }

  fprintf(stderr, "Composite is done\n");
}
